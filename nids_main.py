...
# --- 1. DATA LOADING (Simulation Mode) ---
@st.cache_data
def load_data():
    """
...
if st.button("Analyze Packet"):
    if 'model' in st.session_state:
        model = st.session_state['model']
        # Match training feature order: Destination_Port, Flow_Duration, Total_Fwd_Packets, Packet_Length_Mean, Active_Mean
        input_data = pd.DataFrame([[80, p_dur, p_pkts, p_len, p_active]], columns=X.columns)
